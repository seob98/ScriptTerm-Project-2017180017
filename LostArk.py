    ...
    def DisplayRaidTeam(self):
        for i, character in enumerate(search_engine.raidTeam_Info):
            ...
            if image_url is not None:  # Add check for None image_url
                ...
                x_position = (i % 3 + 1) * 800 / 3.5 - 210
                y_position = start_y_position + (i // 3) * 280
                self.canvas.create_window(x_position, y_position, window=img_label, anchor='nw')

                radio_button = Radiobutton(self.page1, text=character['CharacterName'], variable=self.currentSelectedCharacterLv, value=character['ItemMaxLevel'],
                                              command=lambda lv=character['ItemMaxLevel']: self.selectCharacter(lv))
                radio_button.place(x=x_position + 70, y=y_position + 240)


            else:
                pass # 예외처리 : 캐릭터 레벨이 너무 낮아 캐릭터 이미지를 제공 안하는 경우
