...
class LostArk:
    ...
    def initPage1Canvas(self):
        ...
        self.scrollbar = Scrollbar(self.page1, orient="vertical",
                                   command=self.canvas.yview)   # 스크롤바 생성, 방향은 수직, 커맨드는 캔버스의 yview로 설정
    ...
    def DisplayRaidTeam(self):
        for i, character in enumerate(search_engine.raidTeam_Info):
            ...
            img_label = Label(self.scrollable_frame, image=img)
            img_label.image = img
            img_label.grid(row=i // 3, column=i % 3 + 1)  # 3개씩 나열
